...
# Fixture pro přípravu na testovnání před každým testem - vyčistí tabulku a
# vloží úkol
...
def seed_test_data(db):
    ...
    db.execute(
        ...
        VALUES (1, 'Odevzdat projekt', 'kurz Engeto', 'nezahájeno')
    """
    )
...
def test_update_task(db):
    db.execute(
        "INSERT INTO ukoly (nazev, popis, stav) VALUES (%s, %s, %s)",
        ("Test název 2", "popis", "nezahájeno"),
    )
    db.execute(
        "UPDATE ukoly SET stav = %s WHERE nazev = %s",
        ("hotovo", "Test název 2"),
    )
...
def test_update_invalid_stav(db):
    with pytest.raises(DatabaseError):
        db.execute(
            "UPDATE ukoly SET stav = %s WHERE id = %s", ("nevalidní", 1)
        )
...
def test_delete_task(db):
    db.execute(
        "INSERT INTO ukoly (nazev, popis, stav) VALUES (%s, %s, %s)",
        ("Test název delete", "popis", "nezahájeno"),
    )
