...
# Chci aby se mi otevřelo připojení a trvalo než se provedou všechny testy
def db_conn():
...
def test_add_task_empty_name(db_conn, monkeypatch, capsys):
    inputy = iter(["", "OK název", "OK popis"])
    # druhý a třetí input nutný pro ukončení testu
    monkeypatch.setattr("builtins.input", lambda _: next(inputy))
    add_task(conn=db_conn)
    captured = capsys.readouterr()
    tasks = get_all_tasks(conn=db_conn)
...
def test_update_task(db_conn, monkeypatch, capsys):
    add_task_to_db(
        "Název test update_task", "Popis test1", "nezahájeno", conn=db_conn
    )
    ...
    assert (
        f"Úkol ID '{task_id}' byl aktualizován, aktuální stav: 'probíhá'"
        in captured.out
    )
...
def test_update_task_fail_id(db_conn, monkeypatch, capsys):
    add_task_to_db(
        "Název fail0 update_task", "Popis0 fail id", "nezahájeno", conn=db_conn
    )
    add_task_to_db(
        "Název fail1 update_task", "Popis1 fail id", "probíhá", conn=db_conn
    )
    task_id_1 = get_filtered_tasks(conn=db_conn)[1][0]
    inputy = iter(["text, no number", str(task_id_1), "hotovo"])
    # druhý a třetí input nutný pro ukončení testu
    monkeypatch.setattr("builtins.input", lambda _: next(inputy))
    update_task(conn=db_conn)
    captured = capsys.readouterr()
    ...
    updated = get_filtered_tasks(conn=db_conn)[0]
    assert (updated[-1] == "nezahájeno")
    # ověření, že stav zůstává stále stejný jako na začátku
...
def test_delete_task_non_numeric_string(db_conn, monkeypatch, capsys):
    ...
    tasks = get_all_tasks(conn=db_conn)
    task_id = next(task[0] for task in tasks if task[1] == "Název test")
    # id pro následné ukončení testu
    inputy = iter(["a", str(task_id)])
    # druhý input nutný pro ukončení testu
    monkeypatch.setattr("builtins.input", lambda _: next(inputy))
    delete_task(conn=db_conn)
    captured = capsys.readouterr()
    tasks_after = get_all_tasks(conn=db_conn)
    ...
    # jeden úkol se druhým inputem vymazal pro ukončení testu
...
def test_delete_non_existent_id(db_conn, monkeypatch, capsys):
    ...
    tasks = get_all_tasks(conn=db_conn)
    task_id = next(task[0] for task in tasks if task[1] == "Název úkolu1")
    # id pro následné ukončení testu
...
# použila jsem na závěr Black Formatter kvůli formátování kódu na max 79 znaků
# na řádek
